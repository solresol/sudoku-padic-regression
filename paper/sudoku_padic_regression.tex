...
|0|_p=0.
\]
This norm makes numbers close when their difference is divisible by a large power of \(p\). In affine \(p\)-adic linear regression, an observation with feature vector \(u\) and target \(b\) has residual \(u^\top x-b\), and an ordinary weighted objective sums terms of the form
\[
\gamma\,|u^\top x-b|_p,
\qquad \gamma>0.
\]
