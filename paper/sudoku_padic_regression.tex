...
Since \(u\in\mathbb Z^n\), \(b\in\mathbb Z\), and \(x\in\mathbb Z_p^n\), each residual lies in \(\mathbb Z_p\) and has norm at most \(1\). Every finite signed objective considered on this domain is therefore bounded. An observation is \emph{positive} when \(\sigma=+1\) and \emph{negative} when \(\sigma=-1\); positive observations will pin coefficients and negative observations will supply bounded rewards. In ordinary (Euclidean) weighted least squares the analogous move fails: inserting negative weights into the quadratic objective can destroy convexity and boundedness, whereas over \(\mathbb Z_p\) every residual norm is bounded by \(1\) and a negative row acts as a bounded reward.

\subsection{Terminology}
\label{sec:terminology}

The construction is described throughout in the operational vocabulary of constraint satisfaction and machine learning: a constraint system is \emph{compiled} into a \emph{dataframe} of observations, positive \emph{wells} \emph{pin} coefficients to their finite domains, and global minimisers \emph{snap} onto those domains. Table~\ref{tab:terminology} maps this vocabulary onto the classical language of penalty-function theory, graph theory, and ultrametric analysis, and points to the statement where each notion first does formal work.
...
\midrule
digit-snapping; snapping & Localisation of the minimiser: a coordinatewise projection onto the finite domain that strictly decreases the objective, as in an exact penalty method & Lemma~\ref{lem:digit-snapping} \\
well (unary, Boolean, digit) & Separable penalty summand \(U_i(t)=\sum_{a\in D_i}|t-a|_p\), minimised exactly on the allowed set \(D_i\) & Lemma~\ref{lem:unary-well} \\
compiler; compilation & Constructive polynomial-time reduction from a constraint system to a signed residual objective & Theorem~\ref{thm:compiler-template} \\
...
pinning weight & Penalty weight \(\lambda_i\) on the unary well of coordinate \(i\) & Theorem~\ref{thm:compiler-template} \\
dataframe; row; synthetic observation & Finite family of signed weighted affine forms; a row \((\sigma,u,b,\gamma)\) contributes \(\sigma\gamma\,|u^\top x-b|_p\) & Definition~\ref{def:signed-dataset} \\
feature vector \(u\); target \(b\) & Coefficient vector and constant term of the affine form \(x\mapsto u^\top x-b\) & Definition~\ref{def:signed-dataset} \\
loss & The objective functional \(L\) to be minimised & Definition~\ref{def:signed-dataset} \\
finite-domain all-different system & Constraint system whose variables range over finite sets \(D_i\), with specified sets of variables required to take pairwise distinct values & Section~\ref{sec:introduction} \\
...
Max-CSP; minimum conflicts & Minimisation of the number of violated constraints when not all can be satisfied simultaneously & Section~\ref{sec:minimum-conflicts-potts} \\
...
clue; carving & Cell whose domain is a singleton \(\{g_i\}\); random deletion of entries from a solved grid to generate instances & Section~\ref{sec:sudoku} \\
local search; restart & Iterative neighbourhood descent on a finite state space; an independent random re-initialisation of that descent & Section~\ref{sec:illustrative-computations} \\
\bottomrule
\end{tabular}
\caption{Correspondence between the applied vocabulary used in this paper and its classical reading. Coined terms are anchored to the statements where they first do formal work; standard CSP and machine-learning terms are glossed for reference.}
