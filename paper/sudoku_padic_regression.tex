...
The preceding CNF uses \(0\) for true and \(1\) for false for both variables. The false values need not coincide. They could be other \(p\)-adic integers, or they could be polynomials if the loss is defined using an ultrametric on a rational-function field.
...
For the polynomial option, work over \(\mathbb Q(T)\), encode \(z_1=\mathrm{false}\) by \(x_1=T\), and encode \(z_2=\mathrm{false}\) by \(x_2=T+1\), while retaining \(0\) as the true value for each variable. Table~\ref{tab:cnf-polynomial-targets} reproduces Table~\ref{tab:cnf-dataframe} with these polynomial targets.
