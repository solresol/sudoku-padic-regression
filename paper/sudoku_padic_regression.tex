...
All signed objectives in this paper are optimised over \(\mathbb Z_p^n\). Restricting to the integral domain keeps every affine residual bounded, \(|\cdot|_p\le 1\), and, once \(p>q\), pins it to an exact \(0/1\) equality indicator on the intended finite domain; this is what makes the snapping analysis residue-based. It is a modelling convenience rather than a boundedness requirement: although a single negative term \(-|x_i-x_j|_p\) is unbounded below over \(\mathbb Q_p^n\), the assembled objectives constructed here remain bounded below on all of \(\mathbb Q_p^n\), because the same weight domination \(\lambda>\Delta\) that drives digit-snapping (Definition~\ref{def:list-dataset}, Lemma~\ref{lem:digit-snapping}) also dominates the total inequality reward.
