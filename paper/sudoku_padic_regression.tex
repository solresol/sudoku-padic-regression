...
The heuristics work directly on the digit-valued, clue-consistent row-permutation state space and minimise \(H_{\mathrm{cb}}\). Concretely, we initialise each row as a random permutation of $1,\dots,9$ consistent with its clues, and then apply row-wise swaps that reduce \(H_{\mathrm{cb}}\). This is a block-local search on the integer state space.
