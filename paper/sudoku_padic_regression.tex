...
\item Polynomial-size positive-only complement expansions for the negative CNF and pairwise all-different rows, supporting a diagnostic comparison with Mihara's digitwise equality regression while the signed rows are retained for valuation and loss reporting (Section~\ref{sec:mihara-digitwise}).
...
Section~\ref{sec:padic-regression-background} fixes notation, works a three-variable list-colouring example and a two-variable CNF example in full, and records a family of integer false labellings with the uniform labelling as its degenerate member and a member whose residual values identify the satisfied literals; a polynomial-valued variant is deferred to Appendix~\ref{app:polynomial-labels}. Section~\ref{construction} states and proves the general template and its corollaries. Section~\ref{sec:minimum-conflicts-potts} treats unsatisfiable instances and the Potts interpretation. Sections~\ref{sec:sudoku} and~\ref{sec:parsimony} specialise the construction to Sudoku. Section~\ref{sec:illustrative-computations} reports the computations and compares Mihara's digitwise regression, while Section~\ref{sec:related-work} locates the construction among valued CSPs and Potts energies. The appendices record the heuristics, their traces, the experiments, the failed powers-of-two encoding, the label and coefficient requirements together with an approximate-negation variant, and a polynomial-valued labelling.
...
\]
The general theorem below (Theorem~\ref{thm:compiler-template}) guarantees that at least one global minimum occurs among the finitely many domain-respecting candidates.
...
Within \(\mathbb Z_p\) itself, the signed clause construction works verbatim for any false labels \(b_i\equiv 1\pmod p\): give a positive literal the coefficient \(+1\), a negated literal the coefficient \(-1\), and take as the forbidden target the sum of the false labels of the positive literals. On Boolean-labelled assignments the residual equals minus the sum of the false labels of the satisfied literals, and a nonempty subset of \(k\) such labels sums to \(k\) modulo \(p\), a unit whenever \(k<p\). Every satisfied clause is therefore rewarded by exactly one; the labels need not be distinct, and the uniform label \(b_i=1\) used above is the degenerate member of the family. Because integer norms are at most \(1\), the domination arguments of Section~\ref{construction} are unaffected. Distinct labels add information rather than correctness: with \(b_i=1+p\,2^{i}\), a subset of \(k\) labels sums to \(k+p\sum_{i}2^{i}\), so distinct subsets have distinct sums---a cardinality collision would require \(p\) to divide a nonzero difference of cardinalities smaller than \(p\)---and the residual value then identifies exactly which literals of a clause are satisfied: the count from the residue modulo \(p\), the set from the binary expansion of the quotient. The width bound \(k<p\) is sharp for any labelling with uniform unit rewards, because the Davenport constant of \(\mathbb Z/p\mathbb Z\) is \(p\): among any \(p\) residues there is a nonempty subset summing to zero modulo \(p\). The companion browser's Boolean-CSP page implements both labellings: switching to \(b_i=1+p\,2^{i}\) leaves every displayed loss unchanged, while its row-evaluation table decodes each clause residual into the satisfied literals it names. Appendix~\ref{app:label-requirements} states the general conditions under which any labels and clause coefficients preserve the clause-count loss---and shows that per-clause coefficients recover the same provenance with a prime that depends only on the clause width, not the variable count, and that value labels extend the construction to multi-valued domains, making each satisfied Sudoku inequality row report the digits it compares---while Appendix~\ref{app:approximate-negation} notes that negation may be encoded with non-negative coefficients. Appendix~\ref{app:polynomial-labels} records a polynomial-valued variant of these labels as a direction for future research.

\section{A general all-different residual-objective template}
\label{construction}

The worked examples can now be put on a general footing.
We work in the $p$-adic integers $\mathbb Z_p$, where every norm is at most $1$. If $p>q$, the difference between any two distinct values in $\{1,\dots,q\}$ has norm $1$. For comparison, the discrete metric \(d_{\mathrm{disc}}(s,t):=\mathbf 1_{s\neq t}\) is also an ultrametric; a discrete version of the construction is stated after the main theorem.
...
We implemented two finite-state local-search solvers: a greedy row-swap search and a discrete Zubarev walk. We also implemented a Mihara-inspired last-digit (modulo-\(p\)) equality fit as a diagnostic comparison, not as a claimed Sudoku solver or as a complete implementation of Mihara's published algorithm. Appendix~\ref{app:heuristics} defines the solver loss, records the two algorithms and a locality lemma for row swaps, and gives traces on a standard puzzle.

The greedy solver simply counts integer conflicts: it does not evaluate a \(p\)-adic valuation numerically. On the digit-restricted state space this count is an exact integer representative of the induced zero-or-one residual loss, with column--box overlaps counted twice. The Zubarev solver uses the same induced loss through the Boltzmann transition rule \(\exp(-\beta\,\Delta L)\); its finite implementation can likewise compute \(\Delta L\) from integer conflicts because all relevant norms have already collapsed to indicators. The Mihara-inspired comparison instead performs modulo-\(19\) last-digit equality fitting on a positive-only expansion: every negative peer row is replaced by the sixteen equalities for nonzero digit differences, and unary-row weights are preserved in the consensus score. Fresh full-rank starts can therefore improve the displayed positive-consensus loss. This corrects the sign mismatch, but the rows remain synthetic constraints rather than noisy samples from one hidden affine response law, so Mihara's probabilistic recovery assumptions and termination analysis do not transfer to Sudoku.

The practical result is improvement without solution. On the standard example puzzle, a reproducible run with seed \(0\) completed \(500\) full-rank starts without a singular sample. The retained positive-complement loss fell from \(21{,}703\) after the first start to \(21{,}436\), against the satisfiable floor \(20{,}718\). The retained coefficient vector nevertheless had \(19\) off-domain entries and violated \(22\) of the \(30\) clues; its original signed-objective audit was \(8{,}423\), and no Sudoku solution was recovered. This is one diagnostic trace rather than a performance benchmark, but it illustrates why a descending consensus chart must not be reported as progress toward a valid Sudoku unless the domain, clue, and peer checks also improve.

On a small sample of \(18\) randomly carved puzzles, the greedy and Zubarev solvers both completed every puzzle, giving \(36\) successful method--puzzle runs. The row-swap search used fewer steps on \(14\) of the \(18\) paired puzzles. Its median step counts were \(537.5\), \(2784\), and \(7040\) at \(36\), \(30\), and \(26\) clues, versus \(4533.5\), \(8751.5\), and \(9911\) for the Zubarev walk. Appendix~\ref{app:heuristics-results} gives the experimental setup, full table, and a representative loss curve. These smoke tests show only that both finite-state procedures can reach completions in this sample; they are not a solver benchmark. Mihara's diagnostic is not included among the \(36\) solver runs.
...
\subsection{Mihara's digitwise regression and positive complement expansion}
...
Signed \(p\)-adic residuals have a direct finite-domain role. Positive synthetic terms pin variables to a finite set; after that, negative terms are bounded rewards for inequality or clause satisfaction. This bounded construction should not be conflated with inserting negative weights into ordinary Euclidean least squares.
...
The same signed-residual construction is not limited to pairwise inequality. After Boolean pinning, every clause of width less than \(p\) becomes one forbidden affine equality, so a single signed term rewards its satisfaction. A positive-only affine encoding can represent the same finite-domain penalty by listing all allowed residual values; the signed formulation's advantage is the compact one-row representation of the forbidden complement within that affine class. Section~\ref{sec:general-labels} shows how variables can instead receive distinct false labels whose residual values identify the satisfied literals without changing the loss landscape, and Appendix~\ref{app:polynomial-labels} records a polynomial-valued variant. No optimisation or verification advantage is claimed for these labellings.

The companion site in Figure~\ref{fig:browser-sudoku-solution} lets readers trace a Boolean CSP from clauses to forbidden affine equalities and a signed residual dataframe, or compile an editable Sudoku into its \(1{,}299\)-row objective and watch search reach the theoretical floor. Its Mihara views expose the corresponding positive complement data, weighted-consensus loss, original signed-loss audit, and continued fresh-start policy rather than presenting consensus improvement as a Sudoku solution.
...
This failure and the labelled residuals of Section~\ref{sec:general-labels} use the same combinatorial fact---a superincreasing set has distinct subset sums---in opposite roles. Here the fact aggregates constraints: it replaces the pairwise penalties of each unit by a single sum residual, which discards most of the objective's structure, and the runs above strand local search in local minima. There the fact annotates residuals: with false labels \(1+p\,2^{i}\) the powers of two occupy \(p\)-adic digits that never affect which residuals vanish or what norm they take, so the loss landscape is identical to the uniform labelling and the structure only makes residual values self-describing. These experiments are therefore evidence that subset-sum structure is better used to label residuals than to replace constraints.
...
Fix a clause \(C\) of width \(s\). As before a variable's true value is \(x_i=0\) and its false value is \(x_i=b_i\); now give literal \(i\) of \(C\) an integer coefficient \(c_{C,i}\), and write \(\epsilon_i=-1\) when the literal is positive and \(\epsilon_i=+1\) when it is negated. Take the forbidden target \(t_C=\sum_{\text{positive }i}c_{C,i}b_i\). On a Boolean-labelled assignment the affine residual is
\[
...
A convenient sufficient form of (R2) is to choose representatives \(r_i\in\{1,\dots,p-1\}\) of \(w_{C,i}\bmod p\) with \(\sum_{i\in C}r_i<p\); every nonempty subset then sums to a positive integer below \(p\). The uniform labelling \(b_i=1\) (all \(r_i=1\)) is the case \(s<p\), and \(b_i=2^{i}\) with per-clause label-sum below \(p\) is another. The width bound is sharp: the Davenport constant of \(\mathbb Z/p\mathbb Z\) is \(p\), so any sequence of \(p\) residues has a nonempty zero-sum subsequence, and no choice of labels or coefficients admits clauses of width \(p\) or more or lowers the prime below the maximum clause width.

Failing (R2) breaks the objective rather than only its bookkeeping. Over \(p=5\) with labels \(b_1=1\), \(b_2=4\), the clause \((z_1\vee z_2)\) has \(b_1+b_2=5\equiv 0\): an assignment satisfying both literals has residual \(-5\) and is rewarded \(|{-5}|_5=\tfrac15\) instead of \(1\), so a doubly-satisfied clause counts as almost falsified and the loss stops tracking the satisfied-clause count.
...
\paragraph{Syndrome rows and exact recovery.}
A clause may also carry several rows. Fix distinct nonzero evaluation points \(\alpha_1,\dots,\alpha_s\) and give clause \(C\) the rows \(j=1,\dots,s\) with coefficients chosen so that \(w^{(j)}_{C,i}\equiv\alpha_j^{\,i-1}\pmod p\), each with weight \(1/s\). The residual of row \(j\) on a Boolean-labelled assignment is \(-\chi(\alpha_j)\), where \(\chi(X)=\sum_{i\in\operatorname{Sat}(x)}X^{\,i-1}\), so the \(s\) residuals are evaluations of the indicator polynomial of the satisfied set, and solving the Vandermonde system over \(\mathbb F_p\) recovers that set exactly. Condition (R2) for row \(j\) says that \(\alpha_j\) avoids the roots of the \(2^{s}-1\) subset polynomials \(\sum_{i\in S}X^{\,i-1}\); each has degree below \(s\), so points can be checked directly, or drawn at random when \(p\) is large against \(s2^{s}\). Under (R2) every row individually rewards satisfaction, so the weights \(1/s\) restore one unit of reward per satisfied clause and the loss identity is unchanged. This realises, inside the integer construction, the syndrome-decoding pattern of Reed--Solomon codes: residuals are syndromes and the satisfied set is recovered by interpolation. The cost is \(s\) rows per clause, so it is optional instrumentation rather than a replacement for the one-row encoding.

\paragraph{Value labels for finite domains.}
The same freedom applies to the multi-valued construction of Theorem~\ref{thm:all-different}. Relabel domain value \(v\) by any \(\ell_v\), pairwise incongruent modulo \(p\); the wells move to the labelled values, and every pairwise inequality residual takes the value \(\ell_v-\ell_w\), a unit precisely when \(v\neq w\), so the conflict count and the snapping argument are unchanged. The standard values \(1,\dots,q\) with \(p>q\) are one such labelling. The informative choice \(\ell_v=v+p\,2^{v}\) makes every satisfied inequality row self-describing: its residual is \((v-w)+p\,(2^{v}-2^{w})\), and when \(p>2(q-1)\) the nearest integer to the residual divided by \(p\) is exactly \(2^{v}-2^{w}\), whose sign orders the pair and whose magnitude---a run of ones from bit \(\min(v,w)\) through bit \(\max(v,w)-1\)---recovers both values. For Sudoku at \(p=17\) this holds for all eighty-one ordered value pairs, and the residual column of the compiled objective then reports the actual digits in each constrained pair of cells without consulting the coefficient vector. Unlike the Boolean provenance schemes above, no larger prime and no bound on the number of variables is needed: the label set depends only on the domain size.

\paragraph{Signs as a presentational choice.} Together the two freedoms show that signed coefficients are inessential. Setting every \(c_{C,i}=1\) turns each clause row into a pure unsigned incidence sum \(\sum_{i\in C}x_i=t_C\); condition (R2) then falls on the signed labels \(\{\epsilon_i b_i\}\), and any zero-sum-free choice---for instance distinct powers of two whose per-clause signed sums stay below \(p\) in absolute value---encodes the clause correctly. A literal's polarity can thus be carried by the coefficient sign or by the label, just as the forbidden-versus-allowed distinction can be carried by a signed weight or by the positive complement of Section~\ref{construction}. This is the positive resolution of the failed unsigned attempt of Appendix~\ref{app:polynomial-labels}: the labels \(T,T+1,T+2,T+3\) collide only because they are not chosen zero-sum-free.

\paragraph{Two limits.}
Two tempting uses of these freedoms are unavailable. First, relabelling cannot assist search: every admissible labelling induces the identical loss landscape on the finite domain, so re-randomising labels or coefficients between restarts changes nothing about which states are local minima---in pointed contrast to Appendix~\ref{app:powers-two}, where changing the encoding did change the landscape, for the worse. The invariance that makes the information channels free makes them useless for escaping minima, by the same token. Second, the freedoms cannot compress rows: a single affine row forbids a single hyperplane, and a union of two forbidden hyperplanes is not a hyperplane, so no choice of labels or coefficients merges two clauses into one row. Attempts to do so by aggregation are precisely the failure recorded in Appendix~\ref{app:powers-two}.
