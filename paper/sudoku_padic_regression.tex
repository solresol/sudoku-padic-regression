...
These are \(p\)-adic machine-learning techniques in this setting because they train the coefficient vector by local moves whose selection is biased toward reducing a loss derived from the signed \(p\)-adic residual objective. In particular, row-swap search is a discrete graph walk on the state graph of clue-consistent row permutations, in the same spirit as earlier graph-walk experiments.
