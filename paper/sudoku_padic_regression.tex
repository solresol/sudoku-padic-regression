...
\begin{abstract}
We study signed, weighted affine \(p\)-adic residual objectives as native encodings of finite-domain constraints. For primes that separate the finite alphabet, sufficiently weighted positive unary rows pin each coefficient to its allowed set, while negative rows reward unequal endpoints or clause satisfaction. A coordinatewise domination theorem places every global minimiser in the finite domain; there the loss is, up to an additive constant, the all-different conflict count or the negative number of satisfied CNF clauses. The threshold problem is NP-hard already at the fixed prime \(p=5\). The negative rows used by these compilers also admit polynomial-size positive-only complement expansions over their attainable affine values. This gives a weighted-consensus input for a comparison inspired by Mihara's modulo-\(p\) stage while the compact signed rows remain available for valuation and loss reporting. Standard Sudoku provides an \(81\)-coefficient case study without a one-hot lift. A greedy row-swap method and a discrete Zubarev walk both solved all \(18\) randomly carved smoke-test puzzles, which were not uniqueness-checked and do not constitute a benchmark. The Mihara-inspired comparison instead uses the positive complement expansion, retains the best weighted consensus across fresh starts, and reports its selected loss separately from the original signed objective; it remains an unbounded-restart comparison, not a claimed Sudoku solver. A client-side implementation exposes the generated dataframes, arithmetic, diagnostics, and searches.
\end{abstract}
...
The contribution is one of expressivity. Finite-domain all-different systems and CNF formulae compile mechanically into signed objectives: sufficiently weighted positive unary rows pin each coefficient to a finite allowed set, and a coordinatewise domination theorem (Theorem~\ref{thm:compiler-template}) shows that every global minimiser lies in that finite domain. There each remaining residual norm is a zero-or-one indicator, so a negative row rewards an unequal pair \(x_i\neq x_j\) or a satisfied clause, and the loss is, up to an additive constant, the conflict count of the all-different system or the negative number of satisfied clauses. The global minimisers are exactly the domain-respecting minimum-conflict assignments, and exactly the satisfying assignments when the instance is satisfiable (Theorem~\ref{thm:all-different}, Corollary~\ref{cor:all-different-csp}).

Two caveats frame that claim. First, once the minimisers have snapped to the finite domain, the \(p\)-adic norms have collapsed to indicators, and a discrete-metric variant of the construction behaves identically (Section~\ref{construction}). The claim is therefore not that \(p\)-adic analysis solves constraint problems better than existing methods; it is that the signed affine \(p\)-adic regression class \emph{contains} these problems on their native variables, without a one-hot lift. One consequence for the class itself is that its threshold decision problem is NP-hard already at the fixed prime \(p=5\) (Corollary~\ref{cor:signed-nphard}). Because a positive objective is the special case in which every sign is \(+1\), this hardness also follows from Mihara's forcing theorem \cite{mihara2026forcing}; the derivation here is elementary and self-contained, using only the clause compiler. Second, the Sudoku computations are smoke tests of finite-state search on the induced loss, not a solver benchmark, and their search components are standard: the permutation state space and within-unit swap neighbourhood follow Lewis's metaheuristic Sudoku formulation \cite{lewis2007metaheuristics}, with rows in place of boxes as the permutation unit.
...
\item A Sudoku case study on the native \(81\)-cell digit space: two finite-state solvers, a greedy row-swap search and a discrete Zubarev walk, both of which completed all \(18\) randomly carved smoke-test puzzles; a negative result for a powers-of-two encoding; and a client-side browser implementation exposing the generated dataframes, loss arithmetic, and searches (Sections~\ref{sec:sudoku}--\ref{sec:illustrative-computations}, Appendices~\ref{app:heuristics}--\ref{app:powers-two}).
\end{itemize}

Section~\ref{sec:padic-regression-background} fixes notation, works a three-variable list-colouring example and a two-variable CNF example in full, and records a polynomial-valued labelling of false clauses as a direction for future work. Section~\ref{construction} states and proves the general template and its corollaries. Section~\ref{sec:minimum-conflicts-potts} treats unsatisfiable instances and the Potts interpretation. Sections~\ref{sec:sudoku} and~\ref{sec:parsimony} specialise the construction to Sudoku, and Section~\ref{sec:illustrative-computations} reports the computations. Section~\ref{sec:related-work} locates the construction among valued CSPs, Potts energies, and Mihara's digitwise regression. The appendices record the heuristics, their traces, the experiments, and the failed powers-of-two encoding.
...
|0|_p=0.
\]
This norm makes numbers close when their difference is divisible by a large power of \(p\). Ordinary weighted least squares normally uses nonnegative weights. Inserting negative coefficients into its quadratic objective can destroy convexity and boundedness, so it is not a useful precedent for the signed objective studied here. In affine \(p\)-adic linear regression, an observation with feature vector \(u\) and target \(b\) has residual \(u^\top x-b\), and the usual positive-weight objective sums terms of the form
\[
\gamma\,|u^\top x-b|_p,
\qquad \gamma>0.
\]
...
\]
The general theorem below guarantees that at least one global minimum occurs among the finitely many domain-respecting candidates.
...
The preceding CNF gives a concrete polynomial-valued encoding over \(\mathbb Q[T]\). Let \(V_{C_i}(x)\) be \(1\) when clause \(C_i\) is false and \(0\) when it is true, and take
\[
P_{C_1}(T)=T,
\qquad
P_{C_2}(T)=T+1.
...
Form
\[
R_x(T)=\prod_{i=1}^{2}\bigl(1-V_{C_i}(x)+V_{C_i}(x)P_{C_i}(T)\bigr)
=\prod_{C_i\text{ false under }x}P_{C_i}(T).
\]
Factoring \(R_x\) recovers the false-clause labels. Degree defines a non-Archimedean absolute value on the rational-function field \(\mathbb Q(T)\): for any fixed \(c>1\) and nonzero \(f,g\in\mathbb Q[T]\),
\[
...
\(d_\infty(F,G):=|F-G|_\infty\) is an ultrametric. For a nonzero polynomial \(R\),
\(\log_c|R|_\infty=\deg R\). Thus degree is the logarithmic objective associated with this degree-at-infinity ultrametric, rather than itself being the metric, and minimising either quantity is equivalent. For the four assignments,
\[
\begin{array}{ccc}
x & R_x(T) & \deg R_x\\
(0,0) & 1 & 0\\
...
\end{array}
\]
so minimising \(\deg R_x\) gives the same unique satisfying assignment as Table~\ref{tab:cnf-candidates}. The degree is exactly the number of false clauses, while factorisation retains their identities. Equivalently, each Boolean failure indicator can be written as the multilinear product of the literal-failure indicators, giving one positive nonlinear residual per clause \cite{deloera2007nullstellensatz}.
...
\begin{corollary}[NP-hardness for the degree-at-infinity objective]
\label{cor:degree-metric-nphard}
Given pairwise coprime linear false-clause labels, minimising \(|R_x|_\infty\), equivalently \(\deg R_x\), over Boolean assignments is NP-hard.
\end{corollary}

\begin{proof}
Apply the construction of Section~\ref{sec:polynomial-labels} to a \(3\)-CNF formula, assigning each clause a distinct linear polynomial. Then \(\deg R_x\) is exactly the number of false clauses. In particular, its minimum is \(0\) if and only if the formula is satisfiable, so a global optimiser would decide \textsc{3-SAT}.
\end{proof}
...
\newblock Expressing combinatorial problems by systems of polynomial equations and Hilbert's Nullstellensatz.
\newblock \emph{Combinatorics, Probability and Computing} 18(4), 551--582 (2009).
\newblock \url{https://doi.org/10.1017/S0963548309009894}
