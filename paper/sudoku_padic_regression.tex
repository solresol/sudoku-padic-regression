...
\end{itemize}
Recent work in \emph{p-Adic Numbers, Ultrametric Analysis and Applications} connects \(p\)-adic methods with regression structure and optimisation \cite{baker2025padic,zubarev2025padic}. Mihara studies a different regression regime: polynomial regression with digitwise noise \cite{mihara2026polyreg}, and a probabilistic linear-regression algorithm that estimates the digits of one hidden affine law from random samples with sparse digitwise corruption \cite{mihara2026digitwise}. Here signed residual terms enforce pairwise distinctness across finite domains. The main theorem turns a finite-domain all-different or list-colouring instance into a signed weighted synthetic \(p\)-adic residual objective whose global minimisers are exactly the domain-respecting assignments with the fewest conflicts, and in the satisfiable case exactly the valid solutions. Relative to \cite{baker2025padic,baker2026nphard}, this replaces structural support results and binary forcing gadgets by a polynomial-time finite-domain reduction with an exact global-minimiser description. Section~\ref{sec:mihara-digitwise} explains why Mihara's digitwise-noise linear-regression algorithm is adjacent but not a solver for the Sudoku or Boolean-CSP objectives studied here.
