...
digit-snapping; snapping & Coordinate retraction onto the finite domain that strictly decreases the objective under the domination condition & Lemma~\ref{lem:digit-snapping} \\
...
\parbox[t]{\linewidth}{\raggedright positive complement\\expansion} & Replacement, on a finite residual range, of a negative reward for avoiding a forbidden equality by positive terms for the allowed equalities, up to an additive constant & Section~\ref{sec:mihara-digitwise} \\
...
\item Polynomial-size positive-only complement expansions for the negative CNF and pairwise all-different rows, supporting a diagnostic comparison with Mihara's digitwise equality regression while the signed rows are retained for valuation and loss reporting (Appendix~\ref{sec:mihara-digitwise}).
