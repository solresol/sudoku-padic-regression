...
\section{From an all-different graph to a regression dataframe}
\label{sec:worked-all-different}

We begin with the finite problem rather than the general theorem. Figure~\ref{fig:list-colouring-example} has three variables. Each variable must take a value from its displayed domain, and adjacent variables must be different. The assignment \(x=(1,2,3)\) is one solution.
...
Figure~\ref{fig:list-colouring-regression} gives the eight observations in compact tuple notation. Each tuple \((\sigma,u,b,\gamma)\) has signed weight \(w=\sigma\gamma\). Table~\ref{tab:list-colouring-dataframe} shows the same observations in dataframe form, with one column for each model coefficient.
...
\centering
\small
\begin{tabular}{@{}crrrr l@{}}
\toprule
Row & $x_1$ & $x_2$ & $x_3$ & Target & Signed weight and role \\
\midrule
...
\]
Because \(e_1,e_2,e_3\) form a basis, choosing one pinning row for each coefficient determines a model. There are only
\[
|D_1|\,|D_2|\,|D_3|=2\cdot2\cdot2=8
\]
relevant three-point supports to try. Three observations suffice here because the intercept is fixed at zero and the selected feature rows form a basis; this is not an appeal to the \(n+1\)-point theorem for unrestricted affine \(p\)-adic regression.
...
\section{A CNF formula as the same kind of data}
\label{sec:worked-cnf}

The construction is mechanical for clauses as well. Consider
\[
\Phi=(z_1)\wedge(\neg z_1\vee z_2),
\]
...
\begin{tabular}{@{}crrrl@{}}
\toprule
Row & $x_1$ & $x_2$ & Target & Signed weight and role \\
\midrule
...
Many finite-domain all-different systems can be viewed as list-colouring problems. Each variable $x_i$ has a finite allowed set $D_i$, and an edge $\{i,j\}$ requires $x_i\neq x_j$. An all-different constraint of arity \(r\) contributes the \(\binom{r}{2}\) pairwise edges of its primal-graph clique. This paper uses that pairwise graph, not the stronger propagation of a global \emph{alldifferent} constraint.
...
The companion implementation at
\begin{quote}\small
\url{https://padic-logic.symmachus.org}
\end{quote}
makes the two main compilation paths inspectable without installing software. All computation runs in the browser, and the interface exposes the coefficient vector, target, sign, weight, and residual value of each synthetic observation rather than presenting only a solver result.

\paragraph{Boolean CSP route.}
At the main URL, a reader can enter a Boolean CSP or load one of the bundled examples. A deterministic compiler expands the supported constraint language into CNF, displays every clause as its single forbidden affine equality \(u_C^\top x=t_C\), adds the positive Boolean wells, and constructs the signed regression dataframe. The browser then exhaustively enumerates the \(2^n\) Boolean hyperplanes in worker threads and reports the theoretical floor, the best loss, the number of floor-attaining assignments, and each residual calculation. For example, the bundled four-person job-assignment problem compiles to \(16\) variables and \(60\) clauses. With \(p=17\) and Boolean-well weight \(61\), the displayed dataset has \(60\) negative clause observations and \(32\) positive well observations; exhaustive search tests all \(65{,}536\) assignments and verifies that the best loss equals the displayed floor.

Where a browser exposes the on-device Prompt API, an optional natural-language front end precedes this exact compiler. The language model does not generate the final residual rows. It extracts a typed assignment schema---entities, allowed values, exactly-one conditions, forbidden combinations, and implications---after which deterministic code validates the schema, expands it into CNF, and constructs the signed \(p\)-adic residual dataset. This gives a natural-language-description \(\rightarrow\) CSP schema \(\rightarrow\) CNF \(\rightarrow\) signed \(p\)-adic regression pipeline while keeping the mathematically consequential stages checkable. Browser support and local-model download are platform-dependent, so this front end is experimental; direct CSP entry remains the reproducible route on browsers without that API.

\begin{figure}[t]
\centering
\begin{tikzpicture}[every node/.style={font=\footnotesize,align=center},>=stealth]
\node[draw,dashed,rounded corners,text width=2.15cm,minimum height=1.15cm] (nl) at (-6,0) {natural-language\\description\\\emph{optional}};
\node[draw,rounded corners,text width=2.15cm,minimum height=1.15cm] (schema) at (-3,0) {typed CSP\\schema\\validated};
\node[draw,rounded corners,text width=1.75cm,minimum height=1.15cm] (cnf) at (0,0) {CNF\\clauses};
\node[draw,rounded corners,text width=2.25cm,minimum height=1.15cm] (rows) at (3,0) {signed residual\\dataframe};
\node[draw,rounded corners,text width=2.1cm,minimum height=1.15cm] (search) at (6,0) {browser\\search and\\row audit};
\draw[->] (nl) -- (schema);
\draw[->] (schema) -- (cnf);
\draw[->] (cnf) -- (rows);
\draw[->] (rows) -- (search);
\end{tikzpicture}
\caption{The browser compiler's trust boundary. The dashed language-model stage is optional and platform-dependent. Schema validation, CNF expansion, residual construction, and search are deterministic and exposed for inspection.}
\label{fig:browser-compiler}
\end{figure}

\paragraph{Sudoku route.}
The dedicated \href{https://padic-logic.symmachus.org/\#sudoku}{Sudoku view} accepts an editable grid and compiles it into the native \(81\)-variable construction. For the bundled \(30\)-clue puzzle, the browser displays \(489\) positive domain-pinning observations and \(810\) negative peer observations, for \(1{,}299\) residual rows at \(p=11\) and pinning weight \(21\). It evaluates the genuine \(p\)-adic norms, displays the theoretical floor \(7{,}758\), and runs either the row-swap or discrete Zubarev-walk heuristic while showing both the regression loss and the corresponding conflict count. A successful run reaches loss \(7{,}758\) and zero peer conflicts, making the loss-floor identity visible row by row as well as in aggregate.
...
The site therefore reproduces the paper's construction-level results: the conversation-to-schema route where the browser supports it, deterministic CSP-to-CNF and CNF-to-residual compilation, row-by-row objective arithmetic, exact Boolean enumeration, and representative Sudoku searches. It is not an archival batch runner for every reported numerical result: the \(36\) method--puzzle runs over \(18\) paired puzzles and the \(50\)-puzzle powers-of-two experiment are reproduced from the versioned scripts, seeds, CSV files, and traces in the code and dataset archives listed below.
...
\section{Relation to Mihara's forcing and digitwise-noise regression}
\label{sec:mihara-digitwise}

\subsection{General forcing at arbitrary primes}
...
\subsection{Digitwise-noise regression}
...
The paper does not propose a competitive Sudoku solver. Sudoku is a finite-domain case study in which the variables, domains, and all-different constraints are explicit. The main result is the compiler theorem: coordinatewise-dominating unary wells permit signed affine interactions while forcing every global minimiser onto the intended finite domain. All-different systems then admit polynomial-time encodings whose global minimisers are exactly the domain-respecting minimum-conflict assignments, and in the satisfiable case exactly the satisfying assignments.
...
The companion site in Section~\ref{sec:interactive-demo} turns both constructions into inspectable browser computations. Readers can trace a Boolean CSP from clauses to forbidden affine equalities and a signed residual dataframe, or compile an editable Sudoku into its \(1{,}299\)-row objective and watch search reach the theoretical floor. The optional natural-language front end goes one step further: it extracts a typed CSP schema from a problem description before the deterministic CSP and residual compilers take over. This makes the modelling chain concrete while keeping the exact compilation stages visible and testable.
