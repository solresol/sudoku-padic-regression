...
\title{Signed p-adic Regression Encodings of Finite-Domain All-Different Systems\\with a Sudoku Case Study}
...
Standard Sudoku is an immediate special case: there is one variable per cell, singleton domains on clue cells, and edges between cells that share a row, column, or box. The resulting construction uses the original $81$ cell variables rather than a one-hot lift to $729$ Boolean indicators. The purpose of the Sudoku section is expository rather than algorithmic: it provides a compact worked example where the variables, domains, and all-different constraints are familiar. The main contribution is the general encoding theorem for finite-domain all-different systems as signed $p$-adic residual objectives; the word ``regression'' refers to the affine residual form of the objective, not to an ordinary positive-weight statistical loss.
\end{abstract}
...
This paper studies finite-domain all-different systems encoded as a single signed synthetic $p$-adic residual objective on digit-valued variables, with Sudoku as a worked example. The emphasis is representational parsimony: one variable per semantic object, the same finite domain as the original problem, and a single $p$-adic objective enforcing both domain membership and pairwise distinctness.
...
\end{itemize}
Recent work in this journal already connects $p$-adic methods with both regression structure and optimisation \cite{baker2025padic,zubarev2025padic}. The novelty here is the signed construction in Section~\ref{construction}, where signed residual terms are used to enforce pairwise distinctness across arbitrary finite domains. In plain language, the main theorem says that a finite-domain all-different or list-colouring instance can be turned into a signed weighted synthetic $p$-adic residual objective whose global minimisers are exactly the domain-respecting assignments with the fewest conflicts, and in the satisfiable case exactly the valid solutions. Relative to \cite{baker2025padic,baker2026nphard}, this moves from structural support results and binary forcing gadgets to a polynomial-time finite-domain reduction with an exact global-minimiser description.

Negative regularisation is unusual in Euclidean linear regression. Classical work on generalized ridge already allowed negative ridge parameters \cite{hua1983negative}. More recent theory shows that the risk-optimal ridge parameter can be zero or negative in overparameterised settings when the geometry of the predictor space already supplies implicit shrinkage \cite{kobak2020negative}, and that negative optimal regularisation can also arise under covariate or regression shift \cite{patil2024oodridge}. Related anti-shrinkage effects appear in Gaussian model-uncertainty formulations that lead to de-regularised least squares \cite{wiesel2008gaussian} and in multivariate climate downscaling, where negative ridge parameters were used to improve reproduced covariance structure \cite{cannon2009negative}. These Euclidean precedents are not the same as the signed residual construction used here, but they show that a negative sign is not pathological in itself. What is specific to the present \(p\)-adic setting is that the ultrametric loss makes the signed terms bounded and indicator-like on the intended finite domain, so they can encode inequality constraints directly.

Viewed from the constraint-programming side, the construction is close in spirit to weighted CSP / Max-CSP penalty functions, graph-colouring energy models, and Ising/Potts/QUBO-style encodings of discrete optimisation, while Sudoku itself is routinely formulated by exact cover, SAT, or \emph{alldifferent} constraints \cite{knuth2000dancing,simonis2005sudoku,lynce2006sudoku}. The point here is not merely that a constraint system is mapped to a scalar objective. Rather, once \(p\) is larger than the finite domain and the unary wells snap the variables to their allowed values, each affine residual \(|x_i-x_j|_p\) collapses to an exact equality/inequality indicator on the original domain. This keeps one semantic variable per original variable while retaining a regression-style affine residual form. The same signed-residual viewpoint also extends beyond pairwise inequality: after Boolean pinning, a \(3\)-CNF clause becomes one forbidden affine equality, so signed terms can reward clause satisfaction one clause at a time.
...
There are many standard ways to solve Sudoku puzzles. Exact-cover formulations solved by Knuth's Algorithm X with dancing links \cite{knuth2000dancing}, constraint-programming formulations built around \emph{alldifferent} constraints \cite{simonis2005sudoku}, and SAT encodings \cite{lynce2006sudoku} are all natural approaches. The purpose of the present construction is different. Sudoku is used here as a compact finite-domain case study where the variables, domains, and all-different constraints are familiar, so the general theorem can be written out completely in a concrete setting.
...
For exposition, it is also useful to write the slightly more redundant expanded objective
\begin{align}
L(x) &=
...
For Sudoku, this bookkeeping is completely explicit. In the deduplicated peer graph, every cell has degree \(20\), so Theorem~\ref{thm:all-different} requires \(\alpha>20\) for the minimal objective \eqref{eq:objective-minimal}; a convenient sufficient choice is \(\alpha=21\). For the expanded objective \eqref{eq:objective}, the corresponding unrestricted claim requires \(\alpha_{\mathrm{clue}}>20\) as well, because the all-digit wells are constant on clue cells. If instead one used a duplicated unit-scope row/column/box objective and counted overlaps separately, each cell would contribute \(8+8+8=24\) incident negative terms, and the corresponding safe unary bound would be \(>24\). That \(>24\) threshold belongs to the duplicated unit-scope objective, not to the deduplicated peer objective \eqref{eq:objective-minimal}.
...
Negative weights in Euclidean squared-loss regression are usually interpreted as anti-shrinkage in selected directions and handled only in specific settings. The present construction uses signed residual terms for a different purpose. Positive synthetic terms first pin variables to a finite set; after that, the negative \(p\)-adic terms are automatically bounded and reduce to rewards for inequality on domain-respecting states.

The point of the paper is not to propose a competitive Sudoku solver. Sudoku is used as a compact finite-domain case study in which the variables, domains, and all-different constraints are completely explicit. The main result is the general encoding theorem: finite-domain all-different systems admit polynomial-time signed \(p\)-adic residual encodings whose global minimisers are exactly the domain-respecting minimum-conflict assignments, and in the satisfiable case exactly the satisfying assignments.

The same signed-residual viewpoint is not limited to pairwise inequality. After Boolean pinning, a \(3\)-CNF clause can also be written as one forbidden affine equality, so signed terms can reward clause satisfaction one clause at a time. In this sense, the paper connects $p$-adic regression-style objectives with a broader class of discrete optimisation problems, not by competing with SAT/CSP technology, but by showing that ultrametric residuals can encode crisp finite-domain constraints directly on the original variable set.
...
The theorem-side objective \eqref{eq:objective} induces an NP-hard global optimisation problem, consistent with the earlier NP-hardness result for $p$-adic regression. The heuristics, however, work directly on the digit-valued, clue-consistent row-permutation state space and minimise the duplicated unit-scope loss \(H_{\mathrm{cb}}\) from Section~\ref{sec:illustrative-computations}. Concretely, we initialise each row as a random permutation of $1,\dots,9$ consistent with its clues, and then apply row-wise swaps that reduce \(H_{\mathrm{cb}}\). This is a block-local search on the integer state space.
...
The Python implementation used for the heuristic experiments, together with the experiment driver used to generate Table~\ref{tab:results} and Figure~\ref{fig:loss}, is available from the author on request and is archived in the source tree used to prepare this manuscript.
...
\newblock Accepted for publication in \emph{p-Adic Numbers, Ultrametric Analysis and Applications} (2026); preprint at arXiv:2602.13278.
