...
\begin{abstract}
This article studies a signed synthetic p-adic residual objective for finite-domain all-different systems and uses Sudoku as a worked example. Given a graph whose vertices carry finite allowed sets and whose edges reward unequal assignments, we build positive synthetic observations that create unary domain wells and negative synthetic observations that reward pairwise inequality. For primes $p>q$, every nonzero difference between values in $\{1,\dots,q\}$ has p-adic norm $1$, so on domain-respecting states the objective is an additive constant plus the edge-conflict count. It follows that the global minimisers are exactly the domain-respecting assignments that maximise the number of unequal edges. When the associated list-colouring instance is satisfiable, the minimisers are precisely its proper list-colourings.
...
All signed objectives in this paper are optimised over \(\mathbb Z_p^n\). Allowing unrestricted \(\mathbb Q_p^n\) would make negative residual terms potentially unbounded below, so the integral domain is part of the construction rather than a cosmetic restriction.
...
\end{itemize}
Recent work in \emph{p-Adic Numbers, Ultrametric Analysis and Applications} already connects $p$-adic methods with both regression structure and optimisation \cite{baker2025padic,zubarev2025padic}. The novelty here is the signed construction in Definition~\ref{def:signed-dataset} and Theorem~\ref{thm:all-different}, where signed residual terms are used to enforce pairwise distinctness across arbitrary finite domains. Equivalently, the main theorem says that a finite-domain all-different or list-colouring instance can be turned into a signed weighted synthetic $p$-adic residual objective whose global minimisers are exactly the domain-respecting assignments with the fewest conflicts, and in the satisfiable case exactly the valid solutions. Relative to \cite{baker2025padic,baker2026nphard}, this moves from structural support results and binary forcing gadgets to a polynomial-time finite-domain reduction with an exact global-minimiser description.

Negative signed residual terms are unusual from the viewpoint of ordinary Euclidean linear regression. Classical work on generalized ridge already allowed negative ridge parameters \cite{hua1983negative}. More recent theory shows that the risk-optimal ridge parameter can be zero or negative in overparameterised settings when the geometry of the predictor space already supplies implicit shrinkage \cite{kobak2020negative}, and that negative optimal regularisation can also arise under covariate or regression shift \cite{patil2024oodridge}. Related anti-shrinkage effects appear in Gaussian model-uncertainty formulations that lead to de-regularised least squares \cite{wiesel2008gaussian} and in multivariate climate downscaling, where negative ridge parameters were used to improve reproduced covariance structure \cite{cannon2009negative}. These Euclidean precedents are not the same as the signed residual construction used here, but they show that a negative sign is not pathological in itself. What is specific to the present \(p\)-adic setting is that the ultrametric residuals are bounded on \(\mathbb Z_p^n\) and become indicator-like on the intended finite domain, so they can encode inequality constraints directly.
...
A related positive hole-filling observation explains why the signed clause reward is less fragile than a positive multiwell encoding. Suppose the clause is instead encoded positively by the failure-sum
\[
...
\]
When \(p>3\) and \(t\in\{0,1,2,3\}\), this positive term is minimised exactly on \(S\). Adding the missing positive well at the excluded value changes \(W_{S_C}\) into \(W_{\{0,1,2,3\}}\), which is constant on Boolean inputs; repeating this for every clause leaves only the unary Boolean pinning. This is not an algorithm for \textsc{3-SAT}, but a structural reason that the signed clause-reward formulation is less fragile than a positive multiwell encoding.

\section{Sudoku as a special case}
There are many standard ways to solve Sudoku puzzles. Exact-cover formulations solved by Knuth's Algorithm X with dancing links \cite{knuth2000dancing}, constraint-programming formulations built around \emph{alldifferent} constraints \cite{simonis2005sudoku}, SAT encodings \cite{lynce2006sudoku}, and complexity-theoretic studies of puzzle variants \cite{yato-seta} are all natural points of comparison. The purpose of the present construction is different. Sudoku is used here as a compact finite-domain case study where the variables, domains, and all-different constraints are familiar, so the general theorem can be written out completely in a concrete setting.
...
This is the $9$-valued analogue of the earlier ``snap to $\{0,1\}$'' construction from \cite{baker2026nphard}: the positive pinning observations $t=k$ for all $k\in D$ create a multiwell objective with minima exactly at $D$.
...
Appendix~\ref{app:powers-two} records an alternative encoding that maps digits to powers of two so that each row, column, or box contributes a single sum residual rather than a family of pairwise terms. In local-search experiments on \(50\) benchmark puzzles, this encoding produced no successful solves --- the optimiser kept finding local minima from which it could not escape to find valid completions.
...
Mihara's recent work on \(p\)-adic manifold learning from impartial games gives a useful adjacent point of comparison \cite{mihara2026manifoldgames}. There, 3-heap Nim is used as a benchmark for estimating a \(p\)-adic zero locus associated with the Grundy function from finite samples. The present Sudoku example is deliberately different: it does not try to infer a hidden game-theoretic set, but starts from known finite-domain constraints and compiles them into signed residual terms. Thus the connection is benchmark design and interpretability, not a change to the all-different encoding theorem.
...
The positive hole-filling observation above is one small example of a broader phenomenon: adding synthetic observations can change, and sometimes erase, the induced optimisation landscape rather than merely strengthening an existing constraint.
