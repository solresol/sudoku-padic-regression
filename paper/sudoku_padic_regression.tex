...
The preceding CNF uses \(0\) for true and \(1\) for false for both variables. The false values need not coincide. They could be distinct integers, or they could be polynomials if the loss is defined using an ultrametric on a rational-function field. For example, work over \(\mathbb Q(T)\), encode \(z_1=\mathrm{false}\) by \(x_1=T\), and encode \(z_2=\mathrm{false}\) by \(x_2=T+1\), while retaining \(0\) as the true value for each variable. Table~\ref{tab:cnf-polynomial-targets} reproduces Table~\ref{tab:cnf-dataframe} with these polynomial targets.
...
The clause-signed rows used throughout this paper do not have this defect, and no per-assignment checking is needed. Give a positive literal on \(z_i\) the coefficient \(+1\), a negated literal the coefficient \(-1\), and take as the forbidden target the sum of the false labels of the positive literals, exactly as in the integer construction. On Boolean-labelled assignments the residual then equals minus the sum of the false labels of the satisfied literals. The row therefore vanishes exactly at the falsifying assignment if and only if no nonempty subset of the clause's false labels sums to zero. Affine labels \(f_i=T+m_i\) with distinct constants \(m_i\) satisfy this condition for every clause, since a nonempty subset of size \(k\) sums to \(kT\) plus a constant: over \(\mathbb Q(T)\) unconditionally, and in characteristic \(p\) for clauses of width below \(p\), the same width bound as the integer construction. (The bound is sharp: the Davenport constant of \(\mathbb Z/p\mathbb Z\) is \(p\), so any \(p\) leading coefficients in \(\mathbb F_p\) admit a nonempty zero-sum subset.) In particular the labels \(T,T+1,T+2,T+3\) support every clause once the rows are signed: for the displayed clause the signed functional \(-x_A+x_B+x_C-x_D\) attains its target \(2T+3\) only at the falsifying assignment.
