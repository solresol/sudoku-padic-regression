...
For more than two variables, the false labels must be chosen together with the clause hyperplanes. Simply continuing the labels \(T,T+1,T+2,T+3\) does not support every direct affine encoding. For example, the raw-value sum for the falsifying assignment of \(\neg A\vee B\vee C\vee\neg D\) is \((T+1)+(T+2)=2T+3\), but the complementary assignment has the same sum \(T+(T+3)=2T+3\). A direct unsigned-sum row therefore cannot distinguish them. Clause-specific signed coefficients may avoid this particular collision, but every allowed assignment must then be checked for unintended intersections with the proposed false hyperplane.

Nor does the degree-at-infinity norm automatically preserve the original clause-count loss: different nonzero polynomial residuals can have different degrees and hence different magnitudes. We have not found a useful application enabled by this construction, including over polynomial rings over finite fields. We include it only as a possible direction for future research.
