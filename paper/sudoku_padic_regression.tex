...
\begin{abstract}
We construct a mechanical compiler from finite-domain all-different systems and CNF formulae to signed, weighted affine \(p\)-adic residual objectives. For primes that separate the finite alphabet, every relevant residual norm is zero or one on the intended domain. Sufficiently weighted positive unary rows pin each coefficient to an allowed value, while negative rows reward unequal endpoints or clause satisfaction. A coordinatewise domination theorem proves that every global minimiser lies in the finite domain; there the loss is, up to an additive constant, the conflict count for all-different systems or the negative number of satisfied clauses for CNF. Each negative CNF row also has a polynomial-size positive-only complement expansion over its attainable affine values; this supports a Mihara-style weighted-consensus comparison while retaining the original signed row for valuation and loss reporting. The resulting optimisation is NP-hard already at the fixed prime \(p=5\). Standard Sudoku gives an \(81\)-coefficient case study without a one-hot lift. We implement two finite-state solvers: a greedy row-swap method that evaluates an integer conflict-count representative of the induced loss, and a discrete Zubarev walk whose Boltzmann rule uses changes in that loss. Both solved all \(18\) randomly carved smoke-test puzzles, which were not uniqueness-checked and do not constitute a benchmark. A direct Mihara digitwise equality-regression comparison remains intrinsically \(p\)-adic but does not solve ordinary Sudoku because the unexpanded rows encode many inequalities rather than samples from one hidden affine law. A client-side implementation exposes the generated dataframes, loss arithmetic, and representative searches.
\end{abstract}
...
\item A polynomial-size positive-only complement expansion of each negative CNF row, supporting a diagnostic comparison with Mihara's digitwise equality regression while the signed row is retained for valuation and loss reporting (Section~\ref{sec:mihara-digitwise}).
...
The finite-field linear algebra in an individual trial is polynomial-time. The published algorithm is not, however, a worst-case polynomial-time method: its modulo-\(p\) routine uses an unbounded restart loop, and Mihara explicitly notes that the process need not terminate. The probability analysis can give useful expected behaviour when the random-sampling, affine-hull, and noise assumptions hold; it does not provide a polynomial termination guarantee for arbitrary input data.
...
The companion browser uses this positive complement expansion for its Boolean-CSP Mihara mode and plots the resulting positive \(p\)-adic loss. It still treats the procedure as an unbounded-restart heuristic rather than a polynomial-time solver. The separate ordinary-Sudoku Mihara comparison continues to apply the direct equality fit to the unexpanded signed dataframe and generally returns coefficients that fail the digit, clue, and peer constraints. A method guaranteed to solve every compiled instance in polynomial time would decide the NP-hard threshold problem of Corollary~\ref{cor:signed-nphard}; Mihara's algorithm makes no such guarantee.
...
The companion site in Figure~\ref{fig:browser-sudoku-solution} lets readers trace a Boolean CSP from clauses to forbidden affine equalities and a signed residual dataframe, or compile an editable Sudoku into its \(1{,}299\)-row objective and watch search reach the theoretical floor.
