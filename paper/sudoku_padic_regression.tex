...
Over \(\mathbb R\) the same signed move fails for two independent reasons: a sufficiently weighted negative row destroys boundedness, and convex penalties cannot pin a variable to a finite non-singleton set. The precise comparison is deferred to Section~\ref{sec:archimedean-comparison}, after the template theorem whose hypotheses it illuminates.
...
non-Archimedean. Transplanting the construction to \(\mathbb{R}\) ---
...
$t=\tfrac12$ --- in neither well --- and it remains so however large
$\lambda$ is taken. Heavily weighted observations at $0$ and $1$ do not
force the fitted coefficient to be Boolean; they force it to be
one-half. The same holds for any finite domain: the convex analogue
...
\paragraph{Design choice: clues as data, not as substitution.}
...
The failure recorded in Table~\ref{tab:powers-two-results} is topological.
